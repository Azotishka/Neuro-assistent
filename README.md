...
## 0.7: локальные модели
Кнопка «Локальный ИИ» открывает встроенный проект Qwen Local 3.9.4. Перенесены каталог моделей, настройки контекста и генерации, диагностика загрузки, кэш, мобильный интерфейс и управление чатами. Это отдельный режим с отдельной историей, а не замена API-провайдера нативного чата. Модели загружаются при первом запуске; в APK веса не включены. Нужен WebGPU в установленном WebView. При отсутствии WebGPU проект показывает сообщение о несовместимости; само наличие APK не гарантирует доступность GPU. Нативные команды и системный вызов ассистента сохранены. OCR, выбор файлов и веб-микрофон не подключены к нативным разрешениям; голос используй на главном экране. При закрытии локального экрана его WebView уничтожается и освобождает ресурсы. Запуск локального инференса в фоне не реализован.

## 0.8 Unified
Основной экран и история теперь Qwen Local. Явные команды Android перехватываются до запуска модели и добавляются в тот же разговор. Голос заполняет поле сообщения, отправку подтверждает пользователь. Android bridge доступен только основному фрейму локального HTTPS-origin, без addJavascriptInterface. Кнопка «Кнопка телефона» открывает назначение системным ассистентом. Конкретный жест/кнопку задают настройки производителя.
Режим ожидания запускается только вручную, показывает уведомление и выключается из него. Это foreground-service для быстрого вызова, не непрерывное прослушивание и не гарантированный фоновый инференс. Локальный WebView сохраняется при сворачивании задачи, пока процесс не завершён системой; при уничтожении освобождается.
API и старые разговоры доступны через «API и настройки» на совместимом нативном экране. Объединённая история относится к локальному ИИ и Android-командам; прежняя нативная история не мигрируется автоматически. Веса моделей скачиваются отдельно, WebGPU WebView необходим. Полная офлайн-доступность зависит от загрузки всех ресурсов и сохранности кэша.
