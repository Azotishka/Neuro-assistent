...
function isPocoX6ProUserAgent(userAgent = "") {
  return POCO_UA.test(String(userAgent));
}

function getPocoDefaults() {
  return {
    modelKey: "fast",
    context: "1536",
    thinking: false,
...
function pocoMaxTokens(key, thinking = false, requested = 0) {
  const caps = {
...
const POCO_PROFILE_VERSION = "1";

return { getPocoDefaults, isPocoX6ProUserAgent, pocoMaxTokens, POCO_PROFILE_VERSION };
})();

const __qwen_device_profile = (() => {
const { isPocoX6ProUserAgent, pocoMaxTokens } = __qwen_poco;
